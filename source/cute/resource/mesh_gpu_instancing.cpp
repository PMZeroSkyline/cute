...
void MeshGPUInstancing::submit_array_buffer(GLenum usage)
{
...
void MeshGPUInstancing::submit_vertex_attrib()
{
...
void MeshGPUInstancing::submit(Mesh* mesh)
{
    array_buffer = std::make_shared<Buffer>(GL_ARRAY_BUFFER);
    array_buffer->bind();
    submit_array_buffer(GL_STATIC_DRAW);
    for (const auto& primitive : mesh->primitives)
    {
